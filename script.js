...
memoryMapBtn.addEventListener('click', showMemoryMap);
memoryMapModal.addEventListener('click', (e) => {
    if (e.target === memoryMapModal) {
        hideMemoryMap();
    }
});
...
function getMessagesFromMapping(conversation, targetMessageId = null) {
    ...
    for (let i = 0; i < pathNodes.length; i++) {
        ...
        if (!isMessageVisible(node, conversation)) {
            continue;
        }
...
//辅助2：将消息的 content 对象转换成具体的HTML内容
...
//处理图片生成提示（prompt container）的点击事件
...
searchInput.addEventListener('keyup', (e) => {
    if (e.key === 'Enter') {
        const searchTerm = sanitizeForSearch(searchInput.value.trim());
...
searchInput.addEventListener('input', () => {
    if (searchInput.value.length > MAX_SEARCH_LENGTH) {
        searchInput.value = searchInput.value.substring(0, MAX_SEARCH_LENGTH);
    }
...
function performSearch(searchTerm) {
    if (searchTerm.length > MAX_SEARCH_LENGTH || !searchTerm) {
        console.warn("Search term is too long or empty.");
        return;
    }
...
chatAreaForScroll.addEventListener('scroll', () => {
